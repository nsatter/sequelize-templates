...
describe(helpers.getTestDialectTeaser("sequelize-auto"), function() {
	...
	var setupModels = function(self, callback) {
		...
		var execString = "node " + path.join(__dirname, "..", "bin", "sequelize-templates") + " g -o \"" + testConfig.directory + "\" -d " + config.database + " -h " + config.host;
		...
		it("the model files...", function(done){
			...
			setupModels(self, function(err, stdout){
				...
				if (self.sequelize.options.dialect === "postgres") {
					expect(stdout.indexOf('SELECT table_name FROM information_schema.tables WHERE table_schema =')).toBeGreaterThan(-1);
					...
					expect(stdout.indexOf('SELECT TABLE_NAME, TABLE_SCHEMA FROM INFORMATION_SCHEMA.TABLES')).toBeGreaterThan(-1);
				} else {
